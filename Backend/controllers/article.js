...
var controller = {
    ...
    upload: (req, res) => {
        ...
        // Comprobar la extensión, solo imagenes, si es válido borrar el fichero
        if(file_ext != 'png' && file_ext != 'jpg' && file_ext != 'jpeg' && file_ext != 'gif'){
            ...
            fs.unlink(file_path, (err) => {
                return res.status(200).send({
                    status: 'error',
                    message: 'La extensión de la imagen no es válido !!!'
                });
            });
    ...
    getImage: (req, res) => {
        ...
        fs.exists(path_file, (exists) => {
            if(exists){
                ...
                return res.status(404).send({
                    status: 'error',
                    message: 'El archivo no existe xc'
                });
            }
        });
    },
    ...
    search: (req, res) => {
        ...
        .exec((err, articles) => {
            ...
            if(err){
                return res.status(500).send({
                    status: 'error',
                    message: 'Error en la petición xc'
                });
            }
            ...
            if(!articles || articles.length <= 0){
                return res.status(404).send({
                    status: 'error',
                    message: 'No hay artículo que coincidan con tu búsqueda XC'
                });
            }
